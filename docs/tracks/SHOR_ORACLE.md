...
`src/shor_oracle/field_arithmetic.rs`; submitted documentation remains
`src/shor_oracle/architecture.mmd` and `src/shor_oracle/memory/`. The trusted
evaluator validates 9024 Fiat-Shamir oracle shots. It does not add public
`P+Q` / `2P` witnesses or hidden extra-modulus field-kernel probes. The scored
source must use reversible in-place `F_31` arithmetic rather than enumerated
point or field lookup tables. The score is
