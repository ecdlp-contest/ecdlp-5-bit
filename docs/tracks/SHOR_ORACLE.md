...
and passes only opaque per-field operands and targets to the editable source.
...
scored source must use reversible in-place `F_31` arithmetic rather than P/Q
subgroup-index tables, direct `aP+bQ` tables, or an enumerated point oracle. The
score is
